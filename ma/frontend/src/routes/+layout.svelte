<script lang="ts">
  ...
  import { Home, Cpu, LayoutDashboard, Database } from '@lucide/svelte';
...
<div class="flex h-screen w-screen overflow-hidden bg-slate-50 dark:bg-zinc-950">
  <!-- Sidebar -->
  <aside class="w-64 border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col z-20">
    <!-- Header -->
    <div class="h-16 flex items-center px-6 border-b border-slate-100 dark:border-zinc-800 gap-3">
      <LayoutDashboard class="h-6 w-6 text-emerald-600 dark:text-emerald-400" />
      <span class="font-bold text-lg text-slate-800 dark:text-slate-100 tracking-wide">MMS DASHBOARD</span>
    </div>
    ...
    <!-- Footer -->
    <div class="p-4 border-t border-slate-100 dark:border-zinc-800 text-xs text-slate-400 dark:text-zinc-500 flex items-center gap-2">
      <Database class="h-4 w-4 text-slate-400 dark:text-zinc-500" />
      <span>Status: API Connected</span>
    </div>
  </aside>
