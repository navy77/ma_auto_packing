...
time_prev_get=os.getenv("TIME_PREV_GET_BOX")
...
influx_measurement=os.getenv('INFLUX_MEASUREMENT_BOX')
mqtt_topic=os.getenv("MQTT_TOPIC_4")
...
class mc_statusbox_store_db:
    def query_influx():
        ...
        try:
            ...
            for i in range(len(mqtt_topic_value)):
                ...
                query = f"SELECT time,broker,modbus,mac_id,topic FROM {influx_measurement} WHERE time > {time_prev_ns} AND time < {time_now_ns} AND topic ='{mqtt_topic_value[i]}' order by time desc"
    ...
    def edit_col(df_influx):
            if df_influx is None:
                ...
                try:
                    ...
                    df_insert = df[['occurred','broker','modbus','mac_id','mc_no','process']]
                    return df_insert
                except Exception as e:
    ...
    def df_to_db(df_insert, newest_time):
                #connect to db
                mcstatus_list = ['occurred','broker','modbus','mac_id','mc_no','process']
                    ...
                    try:
                        if not df_insert is None:
                            ...
                            for index, row in df.iterrows():
                                ...
                                insert_string = f"""
                                    INSERT INTO {table_statusbox_name} (registered, occurred, broker, modbus, mac_id, mc_no, process)
                                    values(
                            ...
                            # with open("./.env","w") as f:
                                for line in lines:
                                    if line.startswith("TIME_PREV_GET_BOX"):  # Update the line with the new time
                                        f.write(f"TIME_PREV_GET_BOX='{new_time}'\n")
...
if __name__ == "__main__":  
    try:
        ...
        df_influx, newest_time = mc_statusbox_store_db.query_influx()
        df_insert = mc_statusbox_store_db.edit_col(df_influx)
        time.sleep(1)
        mc_statusbox_store_db.df_to_db(df_insert, newest_time)
